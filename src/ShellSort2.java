...
public class ShellSort2 {
    ...
    public ShellSort2(String auto, int size){
    ...
    public void sortShell() {
        ...
        while (h > 0) {
            for (int i = h; i < students.length; i++) {
                ...
                int j = i;
                while (j >= h && key.getHeight() > students[j - h].getHeight()) {
        ...
        while (h > 0) {
            for (int i = h; i < students.length; i++) {
                Student key = students[i];
                int j = i;
                while (j >= h && (key.getHeight() == students[j - h].getHeight())&&key.getWeight()<students[j-h].getWeight()) {
                    students[j] = students[j - h];
                    j = j - h;
                }
                students[j] = key;
